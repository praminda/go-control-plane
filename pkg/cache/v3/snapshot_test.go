...
func TestSnapshotConsistent(t *testing.T) {
	...
	if snap := cache.NewSnapshot(version, []types.Resource{testEndpoint}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil); snap.Consistent() == nil {
	...
	if snap := cache.NewSnapshot(version, []types.Resource{resource.MakeEndpoint("missing", 8080)},
		[]types.Resource{testCluster}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil); snap.Consistent() == nil {
		t.Errorf("got consistent snapshot %#v", snap)
	}
	if snap := cache.NewSnapshot(version, nil, nil, nil, []types.Resource{testListener}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil); snap.Consistent() == nil {
		t.Errorf("got consistent snapshot %#v", snap)
	}
	if snap := cache.NewSnapshot(version, nil, nil,
		[]types.Resource{resource.MakeRoute("test", clusterName)}, []types.Resource{testListener}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil); snap.Consistent() == nil {
		t.Errorf("got consistent snapshot %#v", snap)
	}
}
